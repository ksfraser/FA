 ...
 *
 * Provides:
 / *@array@* /function select_table($fieldlist = "*", / *@array@* /$where = null, / *@array@* /$orderby = null, / *@int@* /$limit = null)
        ...
        / *@int@* /function count_filtered($where)
 * 
 *
 * *******************************************************************************************/
class table_interface
{
	...
	/**********************************************************//**
	*
	* @param array where variables and (optional) values
	* @return string WHERE clause
	**************************************************************/
	/*@string@*/function build_where( $where_array )
	{
		$whereset = false;
		foreach( $where_array as $key => $value )
		{
			...
			if( strlen( $value ) < 1 )
			{
				//If we pass in variable names as a 1D array assumption is we have the variable set in the object
				if( isset( $this->$key ) )
				{
					$value = $this->$key;
				}
				//else we have a field=field being passed in
				else
					$value = $key;
			}
	...
	/**********************************************************//**
	*
	* @param array orderby variables
	* @return string orderby clause
	**************************************************************/
	/*@string@*/function build_orderby( $orderby_array )
	{
		$orderset = FALSE;
	 ...
	 * @param array string of fields to select 
	...
	*******************************************************************************************************************/
	/*@mysql_result@*/function select_table(/*string*/$fieldlist = "*", /*@array@*/$where = null, /*@array@*/$orderby = null, /*@int@*/$limit = null)
	{
		$sql = "SELECT " . $fieldlist . " from `" . $this->table_details['tablename'] . "`";
		if( isset( $where ) )
		{
			$WHERECLAUSE = $this->build_where( $where );
		}
		if( isset( $orderby ) )
		{
			$ORDERBYCLAUSE = $this->build_orderby( $orderby );
		}
		if( isset( $limit ) )
			$LIMITCLAUSE .= " LIMIT " . $limit;
		$sql .= $WHERECLAUSE . $ORDERBYCLAUSE . $LIMITCLAUSE;
		$res = db_query( $sql, "Couldn't select from " . $this->table_details['tablename'] );
		return $res;
	}
	/***************************************************************************************//**
