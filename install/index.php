...
elseif (isset($_POST['db_test'])) {
	...
	elseif ($_POST['dbname']=='') {
		...
		$_SESSION['inst_set'] = array_merge($_SESSION['inst_set'], array(
			...
			'sel_langs' => RequestService::checkValueStatic('sel_langs'),
			'sel_coas' => RequestService::checkValueStatic('sel_coas'),
			'collation' => $_POST['collation'],
		));
		if (install_connect_db()) {
			$_POST['Page'] = RequestService::checkValueStatic('sel_langs') ? 3 :
				(RequestService::checkValueStatic('sel_coas') ? 4 : 5);
		}
	}
	if (!file_exists($path_to_root . "/lang/installed_languages.inc")) {
