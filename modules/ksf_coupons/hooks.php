 ...
 *
 * ***********************************************************************************/
class hooks_ksf_coupons extends hooks {
	var $module_name = 'ksf_coupons'; 
	...
	*/
	function install_options($app) {
		...
		switch($app->id) {
			...
			//case 'stock':
			case 'orders':
				$app->add_rapp_function(2, _('ksf_coupons'), 
					$path_to_root.'/modules/ksf_coupons/ksf_coupons.php', 'SA_ksf_coupons');
		}
	}

	function install_access()
	{
		$security_sections[SS_ksf_coupons] = _("ksf_coupons");

		$security_areas['SA_ksf_coupons'] = array(SS_ksf_coupons|101, _("ksf_coupons"));

		return array($security_areas, $security_sections);
	}
	function db_postwrite(&$cart, $trans_type)
	{
		//display_notification( "ksf_coupons hooks was told about " . $trans_type );
...
//		return get_extern_rate($curr, $provider, $date);
	...
	{
		require_once( 'class.ksf_coupons.php' );
		$cl = new ksf_coupons( $ksf_coupons_prefsDB );
		$cl->module_install();
		return true;
	}
	 */
