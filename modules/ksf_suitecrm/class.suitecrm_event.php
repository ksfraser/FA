...
class suitecrm_event extends suitecrm
{
	var $id;
//	protected $event_id;
	protected $name;
    ...
    {
	    parent::__construct( $url, $username, $password, "Event" );
    }
	function create()
	{
		try {
	...
	/**************************************//**
	 * Attach a document to the event
	 *
	 * Assumption that CREATE has already been run
	 * so that event_ID is set.
	 *
	 * ***************************************/
	function attach()
	{
	...
	{
		$this->upload_method = "set_event_attachment";
		$this->attach_to_id = $this->event_id;
...
/*
 * The following code creates a event within SuiteCRM.  It does not associate it to any
 * Accounts, Contacts or other records!
 * */
/*
$cl = new suitecrm_event("http://fhsws001/devel/fhs/SuiteCRM/service/v4_1/rest.php", "kevin", "Letmein1" );
$cl->set( "name", "Test Note" );
$cl->set( "description", "This is a Test Note description to ensure we can create a event and attachment." );
$cl->set( "revision", "1" );
$cl->set( "save_filename", "class.suitecrm_event.php" );
$cl->set( "file_upload_path", "class.suitecrm_event.php" );
