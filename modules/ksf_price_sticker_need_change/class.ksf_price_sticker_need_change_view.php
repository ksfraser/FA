...
class ksf_price_sticker_need_change_view extends generic_fa_interface_view {
	...
	function __construct( $prefs,  )
	{
		...
		$this->tabs[] = array( 'title' => 'ksf_price_sticker_need_change Updated', 'action' => 'form_ksf_price_sticker_need_change_completed', 'form' => 'form_ksf_price_sticker_need_change_completed', 'hidden' => TRUE );
		$this->tabs[] = array( 'title' => 'Update ksf_price_sticker_need_change', 'action' => 'form_ksf_price_sticker_need_change', 'form' => 'form_ksf_price_sticker_need_change', 'hidden' => FALSE );
	...
	function usage_form()
	{
		...
		label_row( ksf_price_sticker_need_change, This tab is where you XYZ );
                table_section_title( Known Bugs );
		label_row( Bug 1 - Edit button, pencil Edit buttons does not actually launch an edit screen. Low priority at this time as I have DB access. Work around is to delete and re-add. See Delete bug below. );
		label_row( Bug 2 - Delete Button, Delete button X doesnt reload the table listing the mappings. Work around is to switch tabs (e.g. come to this tab) and return." );
		label_row( "Bug 3 - Configuration", "Config screen doesnt display any of the config variables. As the only one this module has is DEBUG, this is very low priority as there is not any non EXCEPTION debugging code currently included );
	...
	function form_ksf_price_sticker_need_change
	{
		$this->call_table( 'form_ksf_price_sticker_need_change_completed', "ksf_price_sticker_need_change" );
	}
	function form_ksf_price_sticker_need_change_completed
	...
	function master_form()
	{
		$title = ksf_price_sticker_need_change;
		$_SESSION['page_title'] = _( = );
		$this->item_form();
		$msg = ksf_price_sticker_need_change;
		$action = $this->action;
	...
	function item_form()
	{
		display_heading(ksf_price_sticker_need_change);
