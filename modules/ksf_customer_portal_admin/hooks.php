<?php
/*
define( 'ModName', 'ksf_customer_portal_admin' );
define ('SS_' . ModName, 111<<8);
$ksf_customer_portal_admin_prefsDB = ModName . "_prefs";
 */
define ('SS_ksf_customer_portal_admin', 111<<8);
$ksf_customer_portal_admin_prefsDB = "ksf_customer_portal_admin_prefs";
 ...
 *
 * ***********************************************************************************/
class hooks_ksf_customer_portal_admin extends hooks {
	var $module_name = 'Expense Claims'; 
	...
	*/
	function install_options($app) {
		...
		switch($app->id) {
			...
			//case 'orders':
			case 'stock':
				$app->add_rapp_function(2, _('Expense Claims'), 
					$path_to_root.'/modules/ksf_customer_portal_admin/ksf_customer_portal_admin.php', 'SA_ksf_customer_portal_admin');
		}
	}

	function install_access()
	{
		$security_sections[SS_ksf_customer_portal_admin] = _("Expense Claims");

		$security_areas['SA_ksf_customer_portal_admin'] = array(SS_ksf_customer_portal_admin|101, _("Expense Claims"));
	...
	{
		require_once( 'class.ksf_customer_portal_admin.php' );
		$cl = new ksf_customer_portal_admin( $ksf_customer_portal_admin_prefsDB );
		$cl->module_install();
		return true;
	}
	 */
...
*/
	...
	function db_prewrite(&$cart, $trans_type)
	{
		...
		//type 10 == payment?
		//type 12 == invoice?
		display_notification( __FILE__ . ":" . __LINE__ . " Writing transaction to database type " . $trans_type );
		var_dump( $cart );
		echo "<br /><br />";
		return true;
	}
