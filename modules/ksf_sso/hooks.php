...
class ksf_sso_app extends application 
{
	function __construct()
	{
		global $path_to_root;
		parent::__construct( ksf_sso, _($this->help_context = ksf_sso ) );
		$this->add_module( _( "Transactions" );
	        $this->add_lapp_function(0, _('Transaction 1'), $path_to_root.'/modules/ksf_sso/manage/transaction1.php?NewTransaction1=Yes', 'SA_ksf_sso_TRAN', MENU_TRANSACTION);
        	$this->add_rapp_function(0, _('Transaction 2'), $path_to_root.'/modules/ksf_sso/manage/transaction2.php?NewTransaction2=Yes', 'SA_ksf_sso_TRAN', MENU_TRANSACTION);

		$this->add_module( _( "Inquiries and Reports" );
	        $this->add_lapp_function(1, _('Report 1'), $path_to_root.'/modules/ksf_sso/manage/report1.php?NewAction1=Yes', 'SA_ksf_sso_REP', MENU_INQUIRY);
        	$this->add_rapp_function(1, _('Report 2'), $path_to_root.'/modules/ksf_sso/manage/report2.php?NewAction2=Yes', 'SA_ksf_sso_REP', MENU_INQUIRY);

		$this->add_module( _( "Maintenance" );
	        $this->add_lapp_function(1, _('Maintenance 1'), $path_to_root.'/modules/ksf_sso/manage/maintenance1.php?NewAction1=Yes', 'SA_ksf_sso_ADMIN', MENU_MAINTENANCE);
        	$this->add_rapp_function(1, _('Maintenance 2'), $path_to_root.'/modules/ksf_sso/manage/maintenance2.php?NewAction2=Yes', 'SA_ksf_sso_ADMIN', MENU_MAINTENANCE);
...
class hooks_ksf_sso extends hooks {
	var $module_name; 
	function __construct()
	{
		$this->module_name = 'ksf_sso';
	}
	...
	*/
	function install_options($app) {
		switch($app->id) {
			...
			//case 'orders':
			case 'stock':
				$app->add_rapp_function(2, _('ksf_sso'), 
				 'modules/ksf_sso/ksf_sso.php', 'SA_ksf_sso');	//example doesn't have $path_to_root in from of mod_rel_path but this may then break!
		}
	}

	function install_access()
	{
		$security_sections[SS_ksf_sso] = _("ksf_sso");

		$security_areas['SA_ksf_sso'] = array(SS_ksf_sso|101, _("ksf_sso"));
		$security_areas['SA_ksf_sso_ADMIN'] = array(SS_ksf_sso|101, _("ksf_sso"));
		$security_areas['SA_ksf_sso_TRAN'] = array(SS_ksf_sso|101, _("ksf_sso"));
		$security_areas['SA_ksf_sso_REP'] = array(SS_ksf_sso|101, _("ksf_sso"));
	...
	function db_prewrite(&$cart, $trans_type)
	{
		...
		//var_dump( $cart );
		/*
		if( require_once( 'class.ksf_sso_model.php' ) )
		{
			$model = new ksf_sso_model( ksf_sso_PREFS, $this );
	...
	{
		$app->add_application(new ksf_sso_app); // add menu tab defined by example_class
	}
	/*
	//
...
//		return get_extern_rate($curr, $provider, $date);
	...
	*/
	...
	function activate_extension($company, $check_only=true)
	{
			...
			'sql/ksf_sso_types.sql' => array('ksf_sso_types'),
			'sql/ksf_sso.sql' => array('ksf_sso'),
