 ...
 * Provides:
        ...
        function form_ksf_sso
        function form_ksf_sso_completed
        function action_show_form()
        function install()
...
class ksf_sso extends generic_fa_interface_controller {
	var $id_ksf_sso;	//!< Index of table
	var $table_interface;
	function __construct( $prefs )
	{
		...
		require_once( 'class.ksf_sso_view.php' );
		$this->view = new ksf_sso_view( $prefs, $this );
		...
		$this->add_submodules();
		require_once( 'class.ksf_sso_model.php' );
		$this->model = new ksf_sso_model( ksf_sso_PREFS, $this );	//defines the tabl
		$this->model->set( found, $this->get( found ) );

	}
	function run()
	{
		if( isset( $_POST['ksf_sso'] ) )
		{
