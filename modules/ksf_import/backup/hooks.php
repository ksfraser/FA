<?php
/*
define( 'ModName', 'ksf_import' );
define ('SS_' . ModName, 111<<8);
$ksf_import_prefsDB = ModName . "_prefs";
 */
//define ('SS_ksf_import', 111<<8);
require_once( 'ksf_import.inc.php' );
$ksf_import_prefsDB = KSF_IMPORT_PREFS;
...
class hooks_ksf_import extends hooks {
	var $module_name = KSF_IMPORT_MODULE_NAME; 
	...
	*/
	function install_options($app) {
		...
		switch($app->id) {
			...
			//case 'orders':
			case 'stock':
				$app->add_rapp_function(2, _( $this->module_name ), 
					$path_to_root.'/modules/ksf_import/ksf_import.php', SA_KSF_IMPORT);
		}
	}

	function install_access()
	{
		$security_sections[SS_KSF_IMPORT] = _( $this->module_name );

		$security_areas['SA_KSF_IMPORT'] = array(SS_KSF_IMPORT|101, _( $this->module_name));
	...
	{
		require_once( 'class.ksf_import.php' );
		$cl = new ksf_import( $ksf_import_prefsDB );
		$cl->module_install();
		return true;
	}
	 */
...
*/
	...
	{
		/*
		echo "<br /> prewrite hook Transaction type: <br />";
		var_dump( $trans_type );
	 */
		return true;
	}
