...
if (isset($_POST['import'])) {
	if (isset($_FILES['imp']) && $_FILES['imp']['name'] != '') {
		...
		while ($data = fgetcsv($fp, 4096, $sep)) {
			if ($lines++ == 0) 
			{
				display_notification("Empty line read.");
				continue;
			}
			...
			if ($type == 'COMMENT') continue; // a comment
			if ($type == 'BOM') {
			...
			if ($type == 'ITEM') {
			    display_notification("ITEM line read.");
			    $dim = 0;
			    /*
			     * 	See class fa_dimension
                            if ($qty != '') {
			    ...
			    $row = get_item( $id );
			    if (!$row) {
			    	    display_notification("Adding Item " .  $id);
				    add_item( $id, $description, $description,
				    ...
				    $i++;
			    	    display_notification("Done Adding Item " .  $id);
			    } else {
				    display_notification("Updating Item " .  $id);
				    update_item( $id, $description, $description, $cat,
					    $_POST['tax_type_id'], $units, $mb_flag, $_POST['sales_account'], $_POST['inventory_account'],
					    $_POST['cogs_account'], $_POST['adjustment_account'], $_POST['assembly_account'], $dim,
					    0, 0, 0);
			...
			if ($type == 'ITEM' || $type == 'KIT' ) {
			    if (isset($price) && $price != "") {
			    	    display_notification("Setting Price for Item " .  $id . " at " . $price . " " . $currency);
			...
			if ($type == 'PRICE') {
			    display_notification("PRICE line read.");
			    if (isset($price) && $price != "") {
			    	    display_notification("Setting Price for Item " .  $id . " at " . $price . " " . $currency);
			        if ($currency == "") $currency = get_company_pref("curr_default");
			        else {
				    ...
				    require_once( 'sales/includes/db/sales_types_db.inc');
				    $sales_type_id = get_sales_type_id($cat);

				$sql = "SELECT * FROM ".TB_PREF."prices	WHERE stock_id='$code' AND sales_type_id='$sales_type_id' AND
					curr_abrev='$currency'";

				$result = db_query($sql,"price could not be retreived");

				$row = db_fetch_row($result);
				if (!$row) add_item_price($code, $sales_type_id, $currency, $price);
				else update_item_price($row[0], $sales_type_id, $currency, $price);
				$pr++;
			    }
			}
			if ($type == 'BUY') {
			    display_notification("BUY line read.");
				display_notification( "BUY line read" );
				$code = db_escape($code);
