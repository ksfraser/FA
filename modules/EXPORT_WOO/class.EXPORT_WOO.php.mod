...
/*
	copied from generic_orders
*/
 	/*@bool@*/function get_purchase_order()
        {
		...
		{
			//purchase_order is a class from /purchasing/includes/po_class.inc
			require_once( '../..//purchasing/includes/po_class.inc' );
			$this->purchase_order = new purch_order();
                        //return FALSE;
		}
                read_po($this->order_no, $this->purchase_order);		//read_po in purchasing/includes/db/po_db.inc
		return TRUE;
        }
 ...
 */
        ...
        function get_id_range()
        {
                ...
                $this->order_no = max((int)$result['max'], $this->lastoid+1);

                return mysql_fetch_assoc($result);
        }

	/*@bool@*/function export_orders()
	{
		/*
		 ...
		 *
		 ******************************************************************/
		$b_res = $this->get_purchase_order();
		if( ! $b_res )	//get failed.  Can't continue
			return FALSE;
