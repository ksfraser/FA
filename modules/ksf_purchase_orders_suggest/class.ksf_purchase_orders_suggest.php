...
class ksf_purchase_orders_suggest extends generic_fa_interface_controller {
	...
	function __construct( $prefs )
	{
		...
		require_once( 'class.ksf_purchase_orders_suggest_view.php' );
		$this->view = new ksf_purchase_orders_suggest_view( $prefs, $this );
		...
		$this->add_submodules();
		require_once( 'class.ksf_purchase_orders_suggest_model.php' );
		$this->model = new ksf_purchase_orders_suggest_model( ksf_purchase_orders_suggest_PREFS, $this );	//defines the tabl
		$this->model->set( found, $this->get( found ) );

	}
	function run()
	{
		if( isset( $_POST['ksf_purchase_orders_suggest'] ) )
		{
