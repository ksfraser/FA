 ...
 * Provides:
        ...
        function form_ksf_purchase_orders_suggest
        function form_ksf_purchase_orders_suggest_completed
        function action_show_form()
        function install()
...
class ksf_purchase_orders_suggest_view extends generic_fa_interface_view {
	var $id_ksf_purchase_orders_suggest;	//!< Index of table
	function __construct( $prefs,  )
	{
		parent::__construct( null, null, null, null, $prefs, , false );	//generic_interface has legacy mysql connection
		...
		$this->tabs[] = array( 'title' => 'ksf_purchase_orders_suggest Updated', 'action' => 'form_ksf_purchase_orders_suggest_completed', 'form' => 'form_ksf_purchase_orders_suggest_completed', 'hidden' => TRUE );
		$this->tabs[] = array( 'title' => 'Update ksf_purchase_orders_suggest', 'action' => 'form_ksf_purchase_orders_suggest', 'form' => 'form_ksf_purchase_orders_suggest', 'hidden' => FALSE );
	...
	function usage_form()
	{
		...
		label_row( ksf_purchase_orders_suggest, This tab is where you XYZ );
                table_section_title( Known Bugs );
		label_row( Bug 1 - Edit button, Edit buttons (pencil) does not actually launch an edit screen. Low priority at this time as I have DB access. Work around is to delete and re-add. See Delete bug below. );
		label_row( Bug 2 - Delete Button, Delete button (X) does not reload the table listing the mappings. Work around is to switch tabs (e.g. come to this tab) and return. );
		label_row( Bug 3 - Configuration, Config screen doesnt display any of the config variables.  As the only one this module has is DEBUG, this is very low priority as there isnt any non EXCEPTION debugging code currently included );
	...
	function form_ksf_purchase_orders_suggest
	{
		$this->call_table( 'form_ksf_purchase_orders_suggest_completed', "ksf_purchase_orders_suggest" );
	}
	function form_ksf_purchase_orders_suggest_completed
	...
	function master_form()
	{
		$title = ksf_purchase_orders_suggest;
		$_SESSION['page_title'] = _( = );
		$this->item_form();
		$msg = ksf_purchase_orders_suggest;
		$action = $this->action;
	...
	function item_form()
	{
		display_heading(ksf_purchase_orders_suggest);
