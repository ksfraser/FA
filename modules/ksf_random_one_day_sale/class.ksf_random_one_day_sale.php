...
/********************************************************************************************
* This module is designed to select at random items to put on sale
*
* Options to include:
*	Which category(ies) to select from on a certain day (themes i.e. tartan tuesday)
*		grouops of categories to support the theme i.e. highland dance, highland dance shoes, highland dance ...
*		include and exclude selector fields.  Include or exlude based on regegular expressions on the cartegory name
*	Sale amount - by percent or absolute amount.
*		have a category by category selection possible
*		have a default value
*	Instore or online only option
*	number of items per day (i.e. 1 instore, and a different online)
*	Do we create a coupon to publish to WOO OR do we set a sale price with start/end dates
*		Coupon options - single use/min purchase/...
*	Publish a blog announcement?
*	Post on social media accounts
*
*********************************************************************************************/
class ksf_random_one_day_sale extends woo_interface {
	var $id_ksf_random_one_day_sale;	//!< Index of table
...
//		//$this->table_details['foreign'][1] = array( 'column' => "stock_id", 'foreigntable' => "woo_prod_variable_master", "foreigncolumn" => "stock_id", "on_update" => "restrict", "on_delete" => "restrict" );
	...
	{
		$this->call_table( 'form_generate_completed', "Generate List of Sale Items" );
	}
	function form_generate_completed
