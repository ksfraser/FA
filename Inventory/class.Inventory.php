...
/*	
		if (isset($_POST['ProcessOrder']) && $this->can_process()) {
		        ...
		        $modified = ($_SESSION['InventoryItems']->trans_no != 0);
		        ...
		        if ($ret == -1)
		        {
		          ...
		          Item not found in stock_master      {
		                        display_error(_("The reference number field has been increased. Please save the document again."));
...
*/
	...
	function create_table_inventory_last_performed()
	{
		...
		$table_details['tablename'] = $this->company_tb_pref . "inventory_last_performed";
		$table_details['index'][0] = array( 'name' => "primary", 'columns' => "stock_id, location" );
	...
	function record_last_inv_date( $stock_id )
	{
		...
		//$this->location,
		//$this->document_date, 
		$sql = "replace into " . TB_PREF . "inventory_last_performed( stock_id, location, inventory_date ) values ( " . $stock_id . ", " . $this->location . ", " . $this->document_date .")";
	...
	function process()
	{
		...
		foreach( $this->line_items as $item )
		{
			if(  $item['quantity'] != 0 )
			{
				...
				//POSSIBLE EXCEPTION if we want to track what we counted of course
				//BUT we should then have a table of counted items.
	...
	function display_adjustment_items($title, $doEdit = TRUE, $showStock = FALSE)
	{
	       	...
	       	if ( $linecount )
		{
	        	foreach ($this->line_items as $stock_item)
		        {
		                ...
		                //$total += ($stock_item->standard_cost * $stock_item->counted);
				...
				*/
					if( $stock_item['stock_id'] != null )
					{
						...
						if( isset( $stock_item['units'] ) )
			                        	...
			                        	label_cell( "" );
	/*
			        		echo __LINE__ .  " DEBUG: showStock is ";
						if( $showStock )
							echo " TRUE ";
						else echo " FALSE ";
						echo "<br />";
...
/*
	        ...
	        $taxes = $inventory->get_taxes(input_num('freight_cost'));
	        $tax_total = display_edit_tax_items($taxes, $colspan, $inventory->tax_included, 2);
...
*/
	                ...
	                if (list_updated('stock_id')) 
			{
		        	...
		        	//$item_info = get_item_edit_info($this->line_items[$id]['stock_id']);
	                	//$dec = $item_info['decimals'];
	                	//$_POST['counted'] = number_format2(0, $dec);
	                	//$_POST['std_cost'] = price_format($item_info["standard_cost"]);
	                	//$_POST['std_cost'] = price_decimal_format($item_info["standard_cost"], $dec2);
			}
	                	//$this->line_items[$id]['units'] = $item_info["units"];
				//qty_cell($this->line_items[$id]['counted'], false, get_qty_dec($this->line_items[$id]['stock_id']));
	        		//qty_cells(null, 'counted', $this->line_items[$id]['counted'], null, null, $dec);
	        	//	qty_cells(null, 'counted', 1, null, null, 0);
	        	//	label_cell("");
	        }
