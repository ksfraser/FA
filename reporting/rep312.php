...
// $ Revision:	2.0 $
// Creator:	Joe Hunt
// Modified:	Kevin Fraser
// date_:	2005-05-19
// Title:	Inventory Planning Zero Supressed
// ----------------------------------------------------------------
$path_to_root="..";
...
define( 'BASESUB', 7 );	//Used for using Last Year's numbers starting with this month last year
			//Assumption this will be run in the last few days of the month to predict 
			//next month's needs
...
//Want to extend to take into consideration SUPPLIERS and their turnaround times
function getPeriods($stockid, $location, $lastyearsnumbers)
{
	if( $lastyearsnumbers )
	{
		$basesub = BASESUB;
		//$date5 = date('Y-m-d');
		$date5 = date('Y-m-d',mktime(0,0,0,date('m')-$basesub - 1,1,date('Y')));
		$date4 = date('Y-m-d',mktime(0,0,0,date('m')-$basesub++,1,date('Y')));
		$date3 = date('Y-m-d',mktime(0,0,0,date('m')-$basesub++,1,date('Y')));
		$date2 = date('Y-m-d',mktime(0,0,0,date('m')-$basesub++,1,date('Y')));
		$date1 = date('Y-m-d',mktime(0,0,0,date('m')-$basesub++,1,date('Y')));
		$date0 = date('Y-m-d',mktime(0,0,0,date('m')-$basesub++,1,date('Y')));
	}
	else
	{
		$date5 = date('Y-m-d');
...
function print_inventory_planning()
{
    ...
    $reponly = $_POST['PARAM_5'];	//Include the +2 for locations, or only replace what we sold
    $lastyearsnumbers = $_POST['PARAM_6'];	//Use last years sales numbers (prep for highland games)
	if ($destination)
	...
	{
		$basesub = BASESUB;
		$per0 = strftime('%b',mktime(0,0,0,date('m')-$basesub++,1,date('Y')));
		$per1 = strftime('%b',mktime(0,0,0,date('m')-$basesub++,1,date('Y')));
		$per2 = strftime('%b',mktime(0,0,0,date('m')-$basesub++,1,date('Y')));
		$per3 = strftime('%b',mktime(0,0,0,date('m')-$basesub++,1,date('Y')));
		$per4 = strftime('%b',mktime(0,0,0,date('m')-$basesub++,1,date('Y')));
	}
	else
	{
	...
	$headers = array(_('Category'), '', $per4, $per3, $per2, $per1, $per0, $calctext,
    ...
    $rep = new FrontReport(_('Monthly Inventory Planning Report Zeros Supressed'), "InventoryPlanning", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
	...
	while ($trans=db_fetch($res))
	{
		...
		$period = getPeriods($trans['stock_id'], $trans['loc_code'], $lastyearsnumbers);
		//$MaxMthSales = Max($period['prd0'], $period['prd1'], $period['prd2'], $period['prd3'], $period['prd4']);
		$MaxMthSales = Max($period['prd0'], $period['prd1'], $period['prd2'], $period['prd3'], $period['prd4']);
		// We want to further extend this so that it looks at re-order levels for each location
		$IdealStockHolding = $MaxMthSales + $calcfactor;
		//$IdealStockHolding = $MaxMthSales * 3;
		$SuggestedTopUpOrder = $IdealStockHolding - $trans['qty_on_hand'] + $custqty - $suppqty;
		//Only print rows that have suggested order value
